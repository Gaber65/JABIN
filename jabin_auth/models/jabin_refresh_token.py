...
class JabinRefreshToken(models.Model):
    ...
    ip_address = fields.Char(string='Issuing IP', help='Client IP from which the token was issued (for audit).')
    user_agent = fields.Char(string='Issuing User-Agent', help='Client User-Agent from which the token was issued.')
    _sql_constraints = [('jti_unique', 'unique(jti)', 'A refresh token with this jti already exists.')]

    @api.model
    def register(self, *, jti: str, user_id: int, expires_at, ip_address: str='', user_agent: str=''):
        if not jti:
            raise ValidationError('Cannot register a refresh token without a jti.')
        return self.create({'jti': jti, 'user_id': user_id, 'expires_at': expires_at, 'ip_address': ip_address or None, 'user_agent': user_agent or None})
    ...
    def revoke(self):
        ...
        _logger.audit('Refresh tokens revoked: count=%d', len(self), extra={'action': 'revoke_refresh_token', 'count': len(self)})
        return True

    @api.model
    def revoke_all_for_user(self, user_id: int) -> int:
        tokens = self.search([('user_id', '=', user_id), ('is_revoked', '=', False)])
        count = len(tokens)
        if count:
            tokens.revoke()
        return count
    ...
    def purge_expired(self) -> int:
        now = fields.Datetime.now()
        expired = self.search([('is_revoked', '=', True), ('expires_at', '<', now)])
        count = len(expired)
        if count:
            expired.unlink()
        return count
    ...
    def to_dict(self) -> dict:
        self.ensure_one()
        return {'id': self.id, 'jti': self.jti, 'user_id': self.user_id.id, 'expires_at': self.expires_at.isoformat() if self.expires_at else None, 'is_revoked': self.is_revoked, 'revoked_at': self.revoked_at.isoformat() if self.revoked_at else None, 'ip_address': self.ip_address or None}