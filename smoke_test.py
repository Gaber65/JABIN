...
from odoo.addons.jabin_core import ResponseBuilder, ApiError, ExceptionMapper, JabinLogger, JsonHelper, DatetimeHelper, PaginationHelper, StringHelper, ValidationHelper, ValidationResult, EmailValidator, PhoneValidator, PasswordValidator, PriceValidator, WeightValidator, UUIDValidator
from odoo.addons.jabin_core.constants.user_types import UserType
from odoo.addons.jabin_core.constants.order_status import OrderStatus
from odoo.addons.jabin_core.constants.payment_status import PaymentStatus
from odoo.addons.jabin_core.constants.delivery_status import DeliveryStatus
from odoo.addons.jabin_core.constants.stock_status import StockStatus
from odoo.addons.jabin_core.constants.notification_types import NotificationType
...
def _load_module_from_file(module_name: str, file_path: str):
    ...
    return mod
_jwt_module = _load_module_from_file('jabin_sprint2_jwt', os.path.join(HERE, 'custom_addons', 'jabin_security', 'utils', 'jwt_utils.py'))
JWTUtils = _jwt_module.JWTUtils
JWTError = _jwt_module.JWTError
_ctx_module = _load_module_from_file('jabin_sprint2_security_context', os.path.join(HERE, 'custom_addons', 'jabin_security', 'utils', 'security_context.py'))
SecurityContext = _ctx_module.SecurityContext
from passlib.context import CryptContext
_CRYPT = CryptContext(schemes=['bcrypt', 'pbkdf2_sha512'], default='bcrypt', deprecated=['pbkdf2_sha512'], bcrypt__rounds=12)
passed = 0
failed = 0
...
check("UserType.from_value('customer') == CUSTOMER", UserType.from_value('customer') == UserType.CUSTOMER)
...
ve = ResponseBuilder.validation_error([ApiError(field='email', message='Email already exists')])
...
print('\n=== ExceptionMapper ===')
from odoo.addons.jabin_core.utils.exception_mapper import ValidationError as VE, AccessError as AE, MissingError as ME
...
access_tok = JWTUtils.encode_access_token(user_id=42, user_type='admin', email='admin@jabin.test', secret=TEST_SECRET)
...
refresh_tok = JWTUtils.encode_refresh_token(user_id=42, user_type='admin', email='admin@jabin.test', secret=TEST_SECRET)
...
check('refresh exp much later than access', refresh_claims['exp'] > claims['exp'] + 3600)
...
expired_tok = JWTUtils.encode_access_token(user_id=1, user_type='customer', email='c@jabin.test', secret=TEST_SECRET, ttl=0)
...
check('get_user_id on bad sub returns None', JWTUtils.get_user_id({'sub': 'not-a-number'}) is None)
check('get_user_id on missing sub returns None', JWTUtils.get_user_id({}) is None)
print('\n=== JWTUtils: secret resolution ===')
tok_explicit = JWTUtils.encode_access_token(1, 'admin', 'a@b.test', secret='explicit-secret-xyz')
try:
...
check('env secret used when no explicit secret', JWTUtils.decode_token(tok_env, secret='env-secret-abc-123') is not None)
del os.environ['JABIN_JWT_SECRET']
print('\n=== SecurityContext: construction ===')
ctx = SecurityContext(user_id=10, user_type='customer', email='cust@jabin.test', roles=['customer'], permissions={'users.read', 'addresses.create'}, token_id='abc123')
...
admin_ctx = SecurityContext(user_id=1, user_type='admin', email='admin@jabin.test', roles=['admin'], permissions=set())
check('admin is_admin True', admin_ctx.is_admin is True)
check('admin has_permission any code (short-circuit)', admin_ctx.has_permission('anything.i.want') is True)
check('admin has_any_permission (short-circuit)', admin_ctx.has_any_permission(['nonexistent.thing']) is True)
check('admin has_all_permissions (short-circuit)', admin_ctx.has_all_permissions(['x.y', 'a.b', 'c.d']) is True)
...
check('both hashes verify same password', _CRYPT.verify(plain, hashed) and _CRYPT.verify(plain, hashed2))
print('\n=== PasswordService: needs_rehash detection ===')
check('fresh bcrypt does not need rehash', _CRYPT.needs_update(hashed) is False)
pbkdf2_hash = _CRYPT.hash('legacy-test-password')
