    ...
    def wrapper(self, *args: Any, **kwargs: Any):
        ...
        if not token:
            if optional:
                ...
                return func(self, *args, **kwargs)
            envelope = ResponseBuilder.unauthorized(message='Authentication required. Provide a Bearer token.', errors=[{'field': 'Authorization', 'message': 'Missing Bearer token.'}])
            return self._build_response(envelope, status=401)
        try:
            claims = JWTUtils.decode_token(token)
        except JWTError as exc:
            envelope = ResponseBuilder.unauthorized(message=str(exc), errors=[{'field': 'Authorization', 'message': str(exc)}])
            return self._build_response(envelope, status=401)
        kind = JWTUtils.get_token_kind(claims)
        if kind != 'access':
            envelope = ResponseBuilder.unauthorized(message='Invalid token type. An access token is required.', errors=[{'field': 'Authorization', 'message': 'Not an access token.'}])
            return self._build_response(envelope, status=401)
        ...
        if not authz_svc.is_account_active(user_id):
            try:
                request.env['jabin.audit.service'].log(action='auth.blocked_inactive', severity='warning', user_id=user_id, summary='Access blocked – account not active')
            except Exception:
                pass
            envelope = ResponseBuilder.forbidden(message='Account is suspended or inactive.')
