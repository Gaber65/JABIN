...
class JabinRole(models.Model):
    ...
    _rec_name = 'code'
    code = fields.Char(string='Role Code', required=True, index=True, help="Unique machine-readable role identifier (e.g. 'order_manager').")
    name = fields.Char(string='Display Name', required=True, help='Human-readable role name.')
    description = fields.Text(string='Description', help='What this role grants and when it should be assigned.')
    sequence = fields.Integer(string='Sequence', default=10, help='Display order in lists.')
    ...
    def _check_code_format(self):
        ...
        for rec in self:
            if rec.code and (not re.match('^[a-z][a-z0-9_]*$', rec.code)):
                raise ValidationError(f"Role code '{rec.code}' must be lowercase snake_case (letters, digits, underscores; starting with a letter).")

    def unlink(self):
        system = self.filtered('is_system')
        if system:
            raise ValidationError(f"Cannot delete system roles: {', '.join(system.mapped('code'))}")
        return super().unlink()
