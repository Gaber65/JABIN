...
class AuthorizationService(models.AbstractModel):
    ...
    def build_context(self, user_id: int, token_id: Optional[str]=None) -> SecurityContext:
        ...
        ctx = SecurityContext(user_id=user_id, user_type=user_type, email=email, roles=roles, permissions=permissions, token_id=token_id)
        _logger.debug('Built security context for user %s: roles=%s perms=%d', user_id, roles, len(permissions))
        return ctx
    ...
    @api.model
    def authorize(self, ctx: SecurityContext, permission_code: Optional[str]=None, any_of: Optional[List[str]]=None, all_of: Optional[List[str]]=None, require_active_account: bool=True) -> bool:
        if not ctx or not ctx.is_authenticated:
            return False
        if require_active_account and ctx.user_id is not None:
            if not self.is_account_active(ctx.user_id):
                _logger.audit('Authorization denied – inactive account: user=%s', ctx.user_id, extra={'user_id': ctx.user_id, 'action': 'authz_denied_inactive'})
                return False
