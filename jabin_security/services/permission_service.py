...
class PermissionService(models.AbstractModel):
    ...
    @api.model
    def create_role(self, code: str, name: str, description: str='', permission_codes: Optional[List[str]]=None, is_system: bool=False) -> Dict[str, Any]:
        ...
        vals = {'code': code, 'name': name, 'description': description, 'is_system': is_system}
        ...
        _logger.audit('Role created: code=%s permissions=%s', code, permission_codes or [], extra={'role_id': role.id, 'action': 'create_role'})
    ...
    @api.model
    def update_role(self, role_id: int, name: Optional[str]=None, description: Optional[str]=None, permission_codes: Optional[List[str]]=None) -> Dict[str, Any]:
        role = self.env['jabin.role'].browse(role_id)
        if not role.exists():
            raise MissingError(f'Role {role_id} not found.')
        vals: Dict[str, Any] = {}
        ...
        if vals:
            role.write(vals)
            _logger.audit('Role updated: id=%s fields=%s', role_id, list(vals.keys()), extra={'role_id': role_id, 'action': 'update_role'})
        return self._role_to_dict(role)
    ...
    def assign_role(self, user_id: int, role_code: str) -> Dict[str, Any]:
        ...
        _logger.audit('Role assigned: user=%s role=%s', user_id, role_code, extra={'user_id': user_id, 'role_code': role_code, 'action': 'assign_role'})
    ...
    def revoke_role(self, user_id: int, role_code: str) -> Dict[str, Any]:
        ...
        _logger.audit('Role revoked: user=%s role=%s', user_id, role_code, extra={'user_id': user_id, 'role_code': role_code, 'action': 'revoke_role'})
    ...
    def _role_to_dict(role) -> Dict[str, Any]:
        return {'id': role.id, 'code': role.code, 'name': role.name, 'description': role.description or None, 'is_system': role.is_system, 'active': role.active, 'permissions': [p.code for p in role.permission_ids], 'user_count': len(role.user_ids)}
