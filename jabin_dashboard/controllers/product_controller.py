...
def _serialize_product(product) -> Dict[str, Any]:
    ...
    return {
        ...
        "main_image": product.main_image.decode('utf-8') if product.main_image else None,
...
class ProductController(BaseApiController):
    ...
    # ------------------------------------------------------------------
    @http.route(
        "/api/catalog/product/<int:product_id>/stock",
        type="http",
        auth="public",
        methods=["POST"],
        csrf=False,
    ...
    # ------------------------------------------------------------------
    @http.route(
        "/api/catalog/product/<int:product_id>/toggle-active",
        type="http",
        auth="public",
        methods=["POST"],
        csrf=False,
